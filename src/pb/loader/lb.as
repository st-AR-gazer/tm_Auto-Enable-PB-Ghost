namespace Loader {
    ...
    void HidePBIcon() {
        ...
        log("Hid PB icon.", LogLevel::Info, 59, "HidePBIcon");
    }

    void ChangePBName() {
        CTrackManiaNetwork@ network = cast<CTrackManiaNetwork>(GetApp().Network);
        if (network.ClientManiaAppPlayground is null) { log("Failed to get playground. Cannot change PB name.", LogLevel::Error, 64, "ChangePBName"); return; }

        CGameDataFileManagerScript@ dfm = network.ClientManiaAppPlayground.DataFileMgr;
        if (dfm is null) { log("Failed to get data file manager. Cannot change PB name.", LogLevel::Error, 67, "ChangePBName"); return; }
        ...
        for (uint i = 0; i < dfm.Ghosts.Length; i++) {
            ...
            if (string(ghost.Nickname).Contains(GetApp().LocalPlayerInfo.Name)) {
                ...
                log("Changed PB name to Personal best", LogLevel::Info, 74, "ChangePBName");
                return;
            }
        }
    }
    ...
    void DownloadPBFromLeaderboardAndLoadLocal(const string&in mapUid) {
        if (!_Game::HasPersonalBest(CurrentMapUID, true)) { log("No personal best found. Cannot download leaderboard PB ghost.", LogLevel::Warn, 81, "DownloadPBFromLeaderboardAndLoadLocal"); return; }
        ...
        if (req.ResponseCode() != 200) {
            log("Failed to fetch replay record, response code: " + req.ResponseCode(), LogLevel::Error, 97, "DownloadPBFromLeaderboardAndLoadLocal");
            ToggleLeaderboardPB();
            return;
        }
        ...
        if (data.GetType() == Json::Type::Null) {
            log("Failed to parse response for replay record.", LogLevel::Error, 104, "DownloadPBFromLeaderboardAndLoadLocal");
            ToggleLeaderboardPB();
            return;
        }

        if (data.GetType() != Json::Type::Array || data.Length == 0) {
            log("Invalid replay data in response.", LogLevel::Error, 110, "DownloadPBFromLeaderboardAndLoadLocal");
            ToggleLeaderboardPB();
            return;
        }
    ...
    void SetPBVisibility(bool shouldShow) {
        ...
        log("PB ghost visibility set to: " + (shouldShow ? "Visible" : "Hidden"), LogLevel::Info, 124, "SetPBVisibility");
    }
